...
export default function IdentityModal({ onAnswer, onLinkWhatsapp, checking, onClose }: Props) {
  ...
  const handleLink = async (e: React.FormEvent) => {
    ...
    if (!result || !result.participant.id) {
      setError('No encontramos participaciones con ese número. ¿Es tu primera vez?')
      setExisting(false)
      return
    }
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#13102e', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 24px 64px rgba(0,0,0,0.65)' }}
      >
        <div className="text-center mb-6">
          <div className="text-4xl mb-3">👋</div>
          <h2 className="text-xl font-700 mb-1" style={{ fontFamily: 'var(--font-display)', color: '#f0eeff' }}>
        ...
        {checking || linking ? (
          <p className="text-center text-sm" style={{ color: 'rgba(224,220,255,0.5)' }}>
          ...
          <form onSubmit={handleLink} className="flex flex-col gap-3">
            <label className="flex flex-col gap-1.5">
              <span className="text-xs font-500" style={{ color: 'rgba(224,220,255,0.55)' }}>
              ...
              <input
                ...
                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#e8e4ff', fontFamily: 'var(--font-mono)' }}
              />
            </label>
            {error && (
