...
export default function AdminPanel({
  ...
  const persistSession = (r: Role, hid: string | null) => {
    try {
      localStorage.setItem(
        SESSION_KEY,
        JSON.stringify({ role: r, helperId: hid }),
      )
    } catch {
      /* ignore */
    }
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const helper = helpers.find(
      (h) => h.whatsapp.trim() === num && h.password === pw,
    )
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-3">
            <label className="flex flex-col gap-1.5">
              ...
              <div
                ...
                style={{
                  ...
                  border: `1px solid ${
                    pwError ? "rgba(239,68,68,0.5)" : "rgba(255,255,255,0.1)"
                  }`,
                  borderRadius: 12,
                }}
              >
                <span
                  className="flex items-center gap-1.5 px-3 text-sm"
                  style={{
            ...
            <button
              type="submit"
              className="rounded-xl py-3 font-600 text-sm"
              style={{
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "rgba(224,220,255,0.4)" }}>
              Gran Rifa Navideña 2024
            </p>
          </div>
        </div>
        ...
        {([
          ...
          <button
            ...
            style={{
              ...
              color: tab === t ? "#a5b4fc" : "rgba(224,220,255,0.4)",
              border:
                tab === t
...
function HelperPanel({
  ...
  const myTicketIds = new Set(
    tickets.filter((t) => t.referred_by === helper.id).map((t) => t.id),
  )
  ...
  const myPayments = payments.filter((p) =>
    p.ticket_ids.some((id) => myTicketIds.has(id)),
  )

  const paidCount = Array.from(byParticipant.values())
    .flat()
    .filter((t) => t.status === "paid").length

  const reservedCount = Array.from(byParticipant.values())
    .flat()
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "rgba(224,220,255,0.4)" }}>
              Mis compradores — Rifas Pro Salud
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      >
        ...
        >
          ...
          <p className="text-xs mb-3" style={{ color: "rgba(251,191,36,0.6)" }}>
            Envíalo a tus compradores por WhatsApp. Todo lo que vendan se asigna
            a ti.
          </p>
          <div className="flex items-center gap-2">
            <a
      ...
      >
        {[
...

          {
            ...
            value: String(byParticipant.size),
            color: "#818cf8",
            icon: "👥",
          },
        ].map((s) => (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                      ...
                      {tks.map((t) => (
                        <span
                          ...
                          style={{
                            ...
                            color: t.status === "paid" ? "#4ade80" : "#fbbf24",
                            fontFamily: "var(--font-mono)",
                            border:
                              t.status === "paid"
                    ...
                    <p
                      ...
                      style={{ color: "rgba(224,220,255,0.35)" }}
                    >
                      Comparte este código SOLO cuando el comprador te envíe la
                      captura del pago por WhatsApp.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    {payment && payment.status === "pending" && (
                      <>
                        <button
                          onClick={() =>
                            onUpdatePayment(payment.id, "approved")
                          }
                        ...
                        <button
                          onClick={() =>
                            onUpdatePayment(payment.id, "rejected")
                          }
                          className="px-3 py-1.5 rounded-lg text-xs font-600"
                          style={{
                            background: "rgba(239,68,68,0.1)",
                    ...
                    <DeleteButton
                      onDelete={() => onDeleteParticipant(pid)}
                      label="comprador"
                    />
                  </div>
                </div>
...
function PaymentsTab({
  ...
  filter: string
  onFilter: (f: any) => void
  onUpdate: (id: string, s: "approved" | "rejected") => void
}) {
  const { toast } = useToast()
        ...
        {(["all", "pending", "approved", "rejected"] as const).map((f) => (
          <button
            ...
            style={{
              ...
              color: filter === f ? "#a5b4fc" : "rgba(224,220,255,0.5)",
              border:
                filter === f
                  ? "1px solid rgba(99,102,241,0.35)"
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className="font-600 text-sm"
                    style={{ color: "#e8e4ff", fontFamily: "var(--font-body)" }}
                  >
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(p.payment_code || "")
                ...
                <p
                  ...
                  style={{ color: "rgba(224,220,255,0.3)" }}
                >
                  Compartilo SOLO cuando el comprador envíe la captura del pago
                  por WhatsApp.
                </p>
              </div>
              {p.status === "pending" && (
          ...
          <thead>
            <tr style={{ background: "rgba(255,255,255,0.04)" }}>
              {[
                ...
                <th
                  key={h}
                  className="text-left px-4 py-3 text-xs font-600"
                  style={{
          ...
          <tbody>
            {filtered.map((t, i) => {
              ...
              return (
                ...
                >
                  <td
                    className="px-4 py-3 font-700"
                    style={{ color: "#e8e4ff", fontFamily: "var(--font-mono)" }}
                  >
                    {String(t.ticket_number).padStart(3, "0")}
                  </td>
                  ...
                  <td
                    ...
                    style={{ color: p ? "#e8e4ff" : "rgba(224,220,255,0.3)" }}
                  >
                  ...
                  <td
                    className="px-4 py-3 text-xs"
                    style={{ color: h ? "#4ade80" : "rgba(224,220,255,0.3)" }}
                  >
    ...
    <div>
      ...
      >
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: "rgba(255,255,255,0.04)" }}>
              {["Nombre", "WhatsApp", "Números", "Vendidos", "Fecha", ""].map(
                (h) => (
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-xs font-600"
            ...
            {participants.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {own
                      .map((t) => String(t.ticket_number).padStart(3, "0"))
                      .join(", ") || "—"}
                  </td>
                  <td
                    className="px-4 py-3 text-xs font-600"
                    style={{ color: paidCount > 0 ? "#4ade80" : "#fbbf24" }}
                  >
                    {own.length > 0 ? `${paidCount}/${own.length}` : "—"}
                  </td>
                  <td
                    className="px-4 py-3 text-xs"
                    style={{ color: "rgba(224,220,255,0.4)" }}
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        <input
          ...
          placeholder="Contraseña de acceso (se la das al familiar)"
          className="w-full rounded-xl px-3 py-2.5 text-sm outline-none mb-3"
          style={{
            background: "rgba(255,255,255,0.05)",
            border: "1px solid rgba(255,255,255,0.1)",
        ...
        {msg && (
          <p
            ...
            style={{ color: msg.startsWith("✅") ? "#4ade80" : "#fca5a5" }}
          >
            {msg}
          </p>
        )}
        <button
          type="submit"
          disabled={busy}
          className="rounded-xl py-2.5 text-sm font-600"
          style={{
      ...
      <div className="flex flex-col gap-3">
        {helpers.length === 0 && (
          <div
            ...
            style={{ color: "rgba(224,220,255,0.3)" }}
          >
            Aún no hay familiares/amigos registrados
          </div>
        )}
        {helpers.map((h) => {
          const soldCount = tickets.filter(
            (t) => t.referred_by === h.id && t.status === "paid",
          ).length

          const reservedCount = tickets.filter(
            (t) => t.referred_by === h.id && t.status === "reserved",
          ).length
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  >
                    <span>
                      🟢 Vendidos:{" "}
                      <strong style={{ color: "#4ade80" }}>{soldCount}</strong>
                    </span>
                    <span>
                      🟡 Apartados:{" "}
